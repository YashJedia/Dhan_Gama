...
const JodiGame = ({ route, navigation }) => {
    ...
    <SafeAreaView className="bg-[#e0f5ff] h-screen py-2">
      <ScrollView className="">
        <View className="flex flex-col bg-blue-400 rounded-lg py-2 mb-4 justify-center items-center space-x-1">
          <Text className="text-white font-pbold text-xl">{"Jodi"}</Text>
        ...
        <View className="bg-[#BBE9FF] px-4 py-6 mx-6 rounded-lg">
          ...
          <View className="my-2">
            <Text className="ml-2 font-pregular">Enter Jodi Digits</Text>
            <InputBox
              // icon={"call"}
              placeholder={"Enter Jodi Digits"}
              handleChangeText={(e) => {
                if (!/^\d*$/.test(e)) {
                  return Alert.alert(
                    "Message",
                ...
                if (/^\d{0,2}$/.test(e)) {
                  ...
                  Alert.alert("Message", "Only two digits are allowed(0-99)");
                }
              }}
